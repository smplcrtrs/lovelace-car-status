...
`,Ae={id:"placeholder",label:"Placeholder",viewBox:"0 0 303 643",panels:["bonnet","boot","door_fl","door_fr","door_rl","door_rr","window_fl","window_fr","window_rl","window_rr"],tyres:ne,render:t=>J`
  <g class="chassis">
    <rect class="body" x=${re} y=${ae} width=${le} height=${ce} rx=${he} />
    <!-- Apertures: revealed when a door rotates away, so the body is never left holed. -->
    ${we.map(t=>J`
      <rect class="aperture" x=${t.x} y=${t.y} width=${pe} height=${t.h} rx="6" />
    `)}
  ...
  ${xe(t,"bonnet",ye)}
  ${xe(t,"boot",_e)}
  ${we.map(e=>((t,e)=>{const i=(t=>({x:"left"===t.side?t.x+16:t.x+6,y:t.y+10,w:22,h:t.h-20}))(e);return J`
    <g
      class="panel"
      data-panel=${e.id}
      data-kind=${oe[e.id]}
    ...
    >
      <rect class="panel-fill" x=${e.x} y=${e.y} width=${pe} height=${e.h} rx="6" />
      <rect
        class="glass"
        data-panel=${e.window}
        data-kind=${oe[e.window]}
        ...
        rx="4"
      />
      <rect class="panel-edge" x=${e.x} y=${e.y} width=${pe} height=${e.h} rx="6" />
    </g>
  `})(t,e))}
  ${t.showTyres?ne.map(e=>((t,e)=>{const i=ve[e],s=t.tyreReading(e)?.value;return J`
    <g class="tyre" data-pos=${e} data-state=${t.tyreState(e)}>
      <rect class="tyre-body" x=${i.x} y=${i.y} width=${be} height=${$e} rx="7" />
      ${s?J`<text class="tyre-value" x=${i.x+be/2} y=${i.y+$e+15}>${s}</text>`:null}
    </g>
  `})(t,e)):null}
`},ke=303,Ce=t=>t.replace(/([MLC])([^MLCZ]*)/gi,(t,e,i)=>{const s=i.trim().split(/[\s,]+/).filter(Boolean).map(Number).map((t,e)=>e%2==0?ke-t:t);return`${e}${s.join(",")}`}),Se=67,Ee=236,Oe=169,Me=230,Pe=10,je="M28,226L63,228L63,350L28,352Z",Le="M28,354L63,352L63,468L28,470Z",Re="M36,238L58,240L58,338L36,340Z",Te="M36,364L58,362L58,456L36,458Z",Ne={x:28,y:226},Ue={x:28,y:354},He="M29,232C19,227,8,229,6,236C4,242,10,247,18,248C25,249,29,245,30,240Z",ze="M64,32C76,26,92,23,104,22L108,38C96,39,82,42,71,46Z",Be="M62,624C74,628,90,630,103,631L106,616C94,615,80,612,69,608Z",De=26,Ie=62,Ze=9,Fe=(t,e)=>({x:t-De/2,y:e,cx:t}),We={fl:Fe(-30,120),rl:Fe(-30,452),fr:Fe(333,120),rr:Fe(333,452)},Ve=["fl","fr","rl","rr"],Ye=[{id:"door_fl",window:"window_fl",side:"left",skin:je,glass:Re,hinge:Ne},{id:"door_rl",window:"window_rl",side:"left",skin:Le,glass:Te,hinge:Ue},{id:"door_fr",window:"window_fr",side:"right",skin:Ce(je),glass:Ce(Re),hinge:{x:ke-Ne.x,y:Ne.y}},{id:"door_rr",window:"window_rr",side:"right",skin:Ce(Le),glass:Ce(Te),hinge:{x:ke-Ue.x,y:Ue.y}}],qe=(t,e,i,s)=>J`
  <g
    class="panel"
    data-panel=${e}
    data-kind=${oe[e]}
...
`,Xe={id:"suv",label:"SUV",viewBox:"-88 0 479 643",panels:["bonnet","boot","door_fl","door_fr","door_rl","door_rr","window_fl","window_fr","window_rl","window_rr"],tyres:Ve,render:t=>J`
  ${t.showTyres?Ve.map(e=>((t,e)=>{const i=We[e],s=t.tyreReading(e),o=i.y+Ie+30;return J`
    <g class="tyre" data-pos=${e} data-state=${t.tyreState(e)}>
      <rect
        class="tyre-body"
        x=${i.x}
  ...
  <g class="chassis">
    ...
    ${Ye.map(t=>J`<path class="aperture" d=${t.skin} />`)}
    <rect
      class="roof"
      x=${Se}
      y=${Ee}
    ...
    <path class="lamp" d=${ze} />
    <path class="lamp" d=${Ce(ze)} />
    <path class="lamp tail" d=${Be} />
    <path class="lamp tail" d=${Ce(Be)} />
  </g>

  ${qe(t,"bonnet","M62,170C60,118,65,72,74,50L229,50C238,72,243,118,241,170Z",170)}
  ${qe(t,"boot","M58,522L245,522C247,548,246,578,242,596C210,602,93,602,61,596C57,578,56,548,58,522Z",522)}
  ${Ye.map(e=>((t,e)=>J`
  <g
    class="panel"
    data-panel=${e.id}
...
`)(t,e))}
  ...
  <path class="mirror" d=${He} />
  <path class="mirror" d=${Ce(He)} />
`},Ge={suv:Xe,placeholder:Ae},Je=t=>Ge[t??"suv"]??Ge.suv,Ke=new Set(["on","open","opening","true","unlocked"]);let Qe=class extends mt{get _openStyle(){return this.config.open_style??"swing"}_colorVars(){const t=this.config.vehicle?.colors??{};return{"--car-body":n(t.body??"grey"),"--car-accent":n(t.accent??"primary"),"--car-open":n(t.open??"amber"),"--car-fault":n(t.fault??"red"),"--car-ok":n(t.ok??"green")}}_context(){const t=this.config.openings??{},e=this.config.tyres??{};return{showTyres:Object.values(e).some(t=>t?.pressure||t?.warning),panelState:e=>((t,e)=>{if(!t||!e?.entity)return"unknown";const i=t.states[e.entity];if(!i)return"unavailable";if("unavailable"===i.state)return"unavailable";if("unknown"===i.state)return"unknown";const s=i.state.toLowerCase();let o=e.open_state?s===e.open_state.toLowerCase():Ke.has(s);return e.invert&&(o=!o),o?"open":"closed"})(this.hass,t[e]),tyreState:t=>{const i=e[t]?.warning;if(!i||!this.hass)return"unknown";const s=this.hass.states[i];return s&&"unavailable"!==s.state?"unknown"===s.state?"unknown":"on"===s.state?"warn":"ok":"unavailable"},tyreReading:t=>{const i=e[t]?.pressure;if(!i||!this.hass)return;const s=this.hass.states[i];return s&&!Number.isNaN(Number(s.state))?{value:s.state,unit:s.attributes.unit_of_measurement}:void 0}}}_ariaLabel(t,e){const i=e.filter(e=>"open"===t.panelState(e));return i.length?`Car: ${i.join(", ").replace(/_/g," ")} open`:"Car: all closed"}render(){if(!this.config)return Q;const t=Je(this.config.vehicle?.preset),e=this._context();return G`
      <svg
        viewBox=${t.viewBox}
        class=${`style-${this._openStyle}`}
        style=${se(this._colorVars())}
        role="img"
  ...
  `,r([$t({attribute:!1})],Qe.prototype,"hass",void 0),r([$t({attribute:!1})],Qe.prototype,"config",void 0),Qe=r([yt("cs-car-graphic")],Qe);let ti=class extends mt{_stateObj(){if(this.hass&&this.item?.entity)return this.hass.states[this.item.entity]}_showMoreInfo(){this.item?.entity&&i(this,"hass-more-info",{entityId:this.item.entity})}render(){if(!this.item)return Q;const t=this._stateObj(),e=this.item.name??t?.attributes.friendly_name??this.item.entity;let i="—";t&&(i=this.hass?.formatEntityState?this.hass.formatEntityState(t):`${t.state}${t.attributes.unit_of_measurement?` ${t.attributes.unit_of_measurement}`:""}`);const s=this.item.color?n(this.item.color):void 0;return G`
  ...
  `,r([$t({attribute:!1})],ti.prototype,"hass",void 0),r([$t({attribute:!1})],ti.prototype,"item",void 0),ti=r([yt("cs-sensor-row")],ti);const ei=new Set(["on","unlocked","open","heat","cool","heat_cool","auto"]),ii={lock:"mdi:lock",toggle:"mdi:toggle-switch",button:"mdi:gesture-tap-button",climate:"mdi:thermostat",action:"mdi:play"};let si=class extends mt{_stateObj(){if(this.hass&&this.item?.entity)return this.hass.states[this.item.entity]}get _unavailable(){if("action"===this.item?.type)return!this.hass;const t=this._stateObj();return!t||"unavailable"===t.state}get _active(){const t=this._stateObj();return!!t&&ei.has(t.state)}_activate(){if(!this.hass||this._unavailable)return;const{type:t,entity:e,confirm:i}=this.item;if(!i||window.confirm(i))switch(t){case"lock":{const t="locked"===this._stateObj()?.state?"unlock":"lock";this.hass.callService("lock",t,{entity_id:e});break}case"toggle":this.hass.callService("homeassistant","toggle",{entity_id:e});break;case"button":this.hass.callService("button","press",{entity_id:e});break;case"climate":case"action":this.item.tap_action?s(this,this.hass,this.item.tap_action,e):e&&s(this,this.hass,{action:"more-info"},e)}}_setTemperature(t){this.hass&&this.item.entity&&this.hass.callService("climate","set_temperature",{entity_id:this.item.entity,temperature:t})}_renderClimate(){const t=this._stateObj(),e=Number(t?.attributes.temperature),i=this.item.min??Number(t?.attributes.min_temp??16),s=this.item.max??Number(t?.attributes.max_temp??30),o=this.item.step??Number(t?.attributes.target_temp_step??.5),n=Number.isFinite(e)?e:i;return G`
    ...
    `}render(){if(!this.item)return Q;const t=this._stateObj(),e=this.item.name??t?.attributes.friendly_name??this.item.entity??this.item.type,i=this.item.color?n(this.item.color):void 0,s=t?G`<ha-state-icon
        ...
        ></ha-state-icon>`:G`<ha-icon
          class="icon"
          .icon=${this.item.icon??ii[this.item.type]??"mdi:car"}
        ></ha-icon>`;return G`
      <div
        class="control"
  ...
  `,r([$t({attribute:!1})],si.prototype,"hass",void 0),r([$t({attribute:!1})],si.prototype,"item",void 0),si=r([yt("cs-control-button")],si);let oi=class extends mt{constructor(){super(...arguments),this.preview=!1}static getConfigElement(){return document.createElement(e)}static getStubConfig(t){return(t=>{const e={name:"Car"};if(!t)return e;const i=Object.keys(t.states),s={};let o="";for(const[t,e]of Object.entries(wt)){const n=At(i,"binary_sensor",e);n&&(s[t]=n,o||=kt(n,e))}Object.keys(s).length&&(e.openings=s);const n=o?i.filter(t=>t.includes(o)):i,r={};for(const[t,e]of Object.entries(xt)){const i=t=>/tire|tyre/.test(t)&&e.some(e=>t.includes(e)),s=n.find(t=>t.startsWith("sensor.")&&/pressure/.test(t)&&i(t)),o=n.find(t=>t.startsWith("binary_sensor.")&&i(t));(s||o)&&(r[t]={...s&&{pressure:s},...o&&{warning:o}})}4===Object.keys(r).length&&(e.tyres=r);const a=[At(n,"sensor",o?["fuel_level","ev_battery_level","state_of_charge","battery_level"]:["fuel_level"]),At(n,"sensor",["odometer","mileage"])].filter(Boolean),l=[],c=At(n,"lock",[""]);c&&l.push({type:"lock",entity:c});const h=At(n,"climate",[""]);h&&l.push({type:"climate",entity:h});const d=n.find(t=>t.startsWith("button.")&&/hazard/.test(t)&&!/horn/.test(t))??At(n,"button",["hazard"]);d&&l.push({type:"button",entity:d,name:"Hazards"});const p=At(n,"button",["force_refresh","refresh","update"]);p&&l.push({type:"button",entity:p,name:"Refresh"});const u={};return a.length&&(u.left=a.map(t=>({entity:t}))),l.length&&(u.below=l),Object.keys(u).length&&(e.regions=u),e})(t)}setConfig(t){this._config=(t=>{Rt(t,Gt);const e=t,i=e.regions??{};for(const[t,e]of Object.entries(i))e?.forEach((e,i)=>Xt(e,t,i));return{...e,openings:Kt(e.openings),regions:i}})(t)}getCardSize(){return 8}getGridOptions(){return{columns:6,min_columns:4,min_rows:6,rows:9}}_items(t){return this._config?.regions?.[t]??[]}_renderItem(t){return Vt(t)?G`<cs-control-button
          .hass=${this.hass}
          .item=${t}
    ...
    `:Q}_renderFlank(t){if(this._items(t).length)return this._renderRegion(t);const e="left"===t?"right":"left";return this._items(e).length?G`<div class="region region-${t}" aria-hidden="true"></div>`:Q}render(){return this._config?G`
  ...
  `,r([$t({attribute:!1})],oi.prototype,"hass",void 0),r([$t({type:Boolean})],oi.prototype,"preview",void 0),r([vt()],oi.prototype,"_config",void 0),oi=r([yt(t)],oi);const ni={door_fl:"Front left door",door_fr:"Front right door",door_rl:"Rear left door",door_rr:"Rear right door",window_fl:"Front left window",window_fr:"Front right window",window_rl:"Rear left window",window_rr:"Rear right window",bonnet:"Bonnet",boot:"Boot",sunroof:"Sunroof"},ri={fl:"Front left",fr:"Front right",rl:"Rear left",rr:"Rear right"},ai={above:"Above the car",left:"Left of the car",right:"Right of the car",below:"Below the car"},li={above:"Runs the full width. Controls here form a button row.",left:"A column beside the car. Drops below it on narrow screens.",right:"A column beside the car. Drops below it on narrow screens.",below:"Runs the full width. Controls here form a button row."},ci={lock:"Lock / unlock",toggle:"Toggle",button:"Button",climate:"Climate setpoint",action:"Custom action"},hi={lock:"mdi:lock",toggle:"mdi:toggle-switch",button:"mdi:gesture-tap-button",climate:"mdi:thermostat",action:"mdi:flash"},di=["binary_sensor","cover","lock","switch","input_boolean","sensor"],pi=t=>({entity:{filter:[{domain:t}]}}),ui={ui_color:{include_none:!0,default_color:"state"}},fi=t=>{const e={};for(const[i,s]of Object.entries(t??{})){const t="string"==typeof s?s:s?.entity;t&&(e[i]=t)}return e},mi=(t,e)=>{const i={};for(const[s,o]of Object.entries(t??{})){if(!o)continue;const t=e?.[s],n="object"==typeof t?{...t}:{};delete n.entity,i[s]=Object.keys(n).length?{entity:o,...n}:o}return i},gi={name:"vehicle",type:"expandable",title:"Appearance",icon:"mdi:palette",schema:[{name:"preset",label:"Body style",selector:{select:{mode:"dropdown",options:Object.values(Ge).map(t=>({value:t.id,label:t.label}))}}},{name:"colors",type:"expandable",title:"Colours",schema:[{name:"body",label:"Body",selector:ui},{name:"accent",label:"Glass and trim",selector:ui},{name:"open",label:"Open",selector:ui},{name:"fault",label:"Fault",selector:ui},{name:"ok",label:"OK",selector:ui}]}]},yi=[{name:"name",label:"Name",selector:{text:{}}},{name:"icon",label:"Icon",selector:{icon:{}}},{name:"color",label:"Colour",selector:{ui_color:{include_none:!0,default_color:"state"}}}],_i=[{name:"entity",label:"Entity",selector:{entity:{}}},...yi,{name:"display",label:"Show as",selector:{select:{mode:"dropdown",options:[{value:"text",label:"Text"},{value:"gauge",label:"Gauge"},{value:"bar",label:"Bar"}]}}},{name:"",type:"grid",schema:[{name:"min",label:"Minimum",selector:{number:{mode:"box"}}},{name:"max",label:"Maximum",selector:{number:{mode:"box"}}}]}];let bi=class extends mt{constructor(){super(...arguments),this.index=0,this.count=1}get _schema(){return Vt(this.item)?(t=>[{name:"type",label:"Control type",selector:{select:{mode:"dropdown",options:Object.keys(ci).map(t=>({value:t,label:ci[t]}))}}},..."action"===t?[{name:"tap_action",label:"Action",selector:{ui_action:{}}}]:[{name:"entity",label:"Entity",selector:{entity:{}}}],...yi,..."climate"===t?[{name:"",type:"grid",schema:[{name:"min",label:"Minimum",selector:{number:{mode:"box"}}},{name:"max",label:"Maximum",selector:{number:{mode:"box"}}},{name:"step",label:"Step",selector:{number:{mode:"box",step:.5}}}]}]:[],{name:"confirm",label:"Confirmation prompt",selector:{text:{}}}])(this.item.type):_i}_summary(){const t=this.item.entity?this.hass?.states[this.item.entity]:void 0;return this.item.name??t?.attributes.friendly_name??this.item.entity??(Vt(this.item)?ci[this.item.type]:"Sensor")}_valueChanged(t){t.stopPropagation();const e={...t.detail.value};for(const[t,i]of Object.entries(e))""!==i&&void 0!==i||delete e[t];i(this,"item-changed",{index:this.index,item:e})}_move(t,e){e.stopPropagation(),i(this,"item-moved",{index:this.index,to:this.index+t})}_remove(t){t.stopPropagation(),i(this,"item-removed",{index:this.index})}render(){const t=Vt(this.item),e=t?this.item.icon??hi[this.item.type]:this.item.icon;return G`
  ...
  `,r([$t({attribute:!1})],bi.prototype,"hass",void 0),r([$t({attribute:!1})],bi.prototype,"item",void 0),r([$t({type:Number})],bi.prototype,"index",void 0),r([$t({type:Number})],bi.prototype,"count",void 0),bi=r([yt("cs-item-row")],bi);let $i=class extends mt{constructor(){super(...arguments),this.items=[]}_emit(t){i(this,"items-changed",{region:this.region,items:t})}_itemChanged(t){t.stopPropagation();const{index:e,item:i}=t.detail,s=[...this.items];s[e]=i,this._emit(s)}_itemRemoved(t){t.stopPropagation(),this._emit(this.items.filter((e,i)=>i!==t.detail.index))}_itemMoved(t){t.stopPropagation();const{index:e,to:i}=t.detail;if(i<0||i>=this.items.length)return;const s=[...this.items],[o]=s.splice(e,1);s.splice(i,0,o),this._emit(s)}_add(t){this._emit([...this.items,t])}render(){return G`
      <ha-expansion-panel outlined .header=${ai[this.region]}>
        <div class="body">
          <p class="hint">${li[this.region]}</p>
  ...
  `,r([$t({attribute:!1})],$i.prototype,"hass",void 0),r([$t({attribute:!1})],$i.prototype,"items",void 0),r([$t()],$i.prototype,"region",void 0),$i=r([yt("cs-region-editor")],$i);let vi=class extends mt{constructor(){super(...arguments),this._computeLabel=t=>t.label??t.name}setConfig(t){this._config=t}get _formData(){return{...this._config,openings:fi(this._config?.openings)}}_emit(t){this._config=t,i(this,"config-changed",{config:t})}_valueChanged(t){if(t.stopPropagation(),!this._config)return;const e=t.detail.value,i={...this._config,...e,openings:mi(e.openings,this._config.openings)};for(const t of["name","open_style"])""===i[t]&&delete i[t];this._emit(i)}_itemsChanged(t){if(t.stopPropagation(),!this._config)return;const{region:e,items:i}=t.detail,s={...this._config.regions,[e]:i};i.length||delete s[e];const o={...this._config,regions:s};Object.keys(s).length||delete o.regions,this._emit(o)}render(){return this._config&&this.hass?G`
      <div class="editor">
        <ha-form
          .hass=${this.hass}
          .data=${this._formData}
          .schema=${(t=>{const e=Je(t.vehicle?.preset);return[{name:"name",label:"Title",selector:{text:{}}},{name:"open_style",label:"How openings are shown",selector:{select:{mode:"dropdown",options:[{value:"swing",label:"Swing open"},{value:"highlight",label:"Highlight only"},{value:"both",label:"Swing and highlight"}]}}},gi,(s=e.panels,{name:"openings",type:"expandable",title:"Doors, windows and openings",icon:"mdi:car-door",schema:s.map(t=>({name:t,label:ni[t],selector:pi(di)}))}),(i=e.tyres,{name:"tyres",type:"expandable",title:"Tyres",icon:"mdi:car-tire-alert",schema:i.map(t=>({name:t,type:"expandable",title:ri[t],schema:[{name:"pressure",label:"Pressure",selector:pi(["sensor","number","input_number"])},{name:"warning",label:"Warning",selector:pi(["binary_sensor","switch","input_boolean"])}]}))})];var i,s})(this._config)}
  ...
  `,r([$t({attribute:!1})],vi.prototype,"hass",void 0),r([vt()],vi.prototype,"_config",void 0),vi=r([yt(e)],vi),wi={type:t,name:"Car Status Card",description:"Top-down car graphic with live door, window, bonnet and boot status.",preview:!0,documentationURL:"https://github.com/smplcrtrs/lovelace-car-status"},window.customCards=window.customCards||[],window.customCards.push(wi),console.info("%c CAR-STATUS-CARD %c 0.1.0 ","color:white;background:#03a9f4;font-weight:700","color:#03a9f4;background:#1c1c1c");
